<template>
  <a-card>
    <a-button @click="getDidcts" type="primary">获取字典</a-button>
    <a-select placeholder="请选择" style="width: 120px">
      <a-select-option v-for="(item) in dicts.value2" :value="item" :key="item">{{item}}</a-select-option>
    </a-select>
    <DictSelect />
...
export default {
  name: "QueryList",
  components: { StandardTable, DictSelect },
  data() {
    return {
      advanced: true,
      columns: columns,
  ...
  computed: {
    dicts() {
      const value = this.$store.getters["account/dicts"];
  ...
  methods: {
    getDidcts() {
      this.$store.dispatch("account/getDictsAction");
    },
    deleteRecord(key) {
